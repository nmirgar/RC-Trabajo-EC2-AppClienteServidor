...
int main(int argc, char *argv[])
{
    ...
    while (!exit)
    {
        ...
        else
        {
			...
			if(strcmp(buffer, "LIST_FILES\n")){
				char* list_files_resutl = list_files();
				printf("\nList files %s\n", list_files_resutl);
				ssize_t len_list_files = strlen(list_files_resutl);
				printf("Post size_T");
				if (send(accepted_socket, list_files_resutl, len_list_files, 0) < 0)
					{
						printf("Send failed\n");
						return 1;
					}
